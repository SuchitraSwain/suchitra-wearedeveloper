/**
 * Records performance demo: Cursor IDE shell + real Chrome/DevTools screenshots.
 * Flow: prompt → MCP connect → Chrome opens → performance_trace → memory snapshot.
 */
...
const recordingUrl = "http://localhost:5173/recording-performance-demo.html"
...
const videoOut = join(publicDir, "devtools-performance-demo.webm")
const posterOut = join(publicDir, "devtools-performance-demo-poster.png")
...
async function runPerformanceTrace(sitePage) {
  const client = await sitePage.context().newCDPSession(sitePage)
  ...
  await sitePage.reload({ waitUntil: "networkidle", timeout: 60000 })
  await sitePage.waitForTimeout(3500)
...
// Show live Chrome + DevTools in the browser panel
...
await caption("Recording a real DevTools performance profile on page load…")
await tool("performance_trace()", "Starting DevTools Performance trace · reload page")
await recordingPage.waitForTimeout(700)

await runPerformanceTrace(sitePage)
;({ sitePage, devtoolsPage } = findPages(cdpBrowser))
await injectComposite(recordingPage, sitePage, devtoolsPage)
...
await agent(
  "Trace complete on live portfolio — LCP and main-thread timeline visible in DevTools Performance panel."
)
await recordingPage.waitForTimeout(900)

await phase("Memory snapshot")
await tool("take_memory_snapshot()", "Capturing heap snapshot from live Chrome tab")
await recordingPage.waitForTimeout(700)
...
await agent("Heap snapshot captured from the live tab — memory state recorded alongside the CPU trace.")
await recordingPage.waitForTimeout(800)

await recordingPage.evaluate(() =>
  window.showComplete("Task complete", [
    "performance_trace on suchitra-swain.web.app · real DevTools CPU timeline",
    "take_memory_snapshot · live heap captured",
    "Agent summarized LCP, main-thread work, and memory from runtime data",
  ])
)
await injectComposite(recordingPage, sitePage, devtoolsPage)
...
if (recordedPath && existsSync(recordedPath)) {
  ...
  console.log(`Saved performance demo video to ${videoOut}`)
} else {
